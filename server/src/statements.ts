...
const colToRoundedAvg = (col: string) => `round(avg(${col}), 2) AS ${col}`;
export const select = {
    instantLatest: (count: number) => `select * from instant order by time desc limit 0,${count}`,
    instantLatestByDay: (day: string) => `select * from instant where time like '${day}%' order by time desc limit 0,144`,
    instantLatest24h: (day: string) => `select * from instant where time > date('now', '-1 day') || ' ' || time('now', '-1 day') order by time desc`,
    ...
    ORDER BY xtime asc;
  `,
    dailyLatest: (count: number) => `select * from daily order by day desc limit 0,${count}`,
    monthlyLatest: (count: number) => `select * from monthly order by month desc limit 0,${count}`,
    yearlyLatest:  (count: number) => `select * from yearly order by year desc limit 0,${count}`,
    hourlyLatest: (count: number) => `select * from hourly order by hour desc limit 0,${count}`,
    instantAverageByRange: (from: string, to: string) => `select time(time),
...
export const ingest = {
        instant: `insert or replace into instant (time, ${commonCols}, thermal, inter, bat, consBat) values(?,${commonPlaceholders},?,?,?,?);`,
        ...
        yearlyInstalled: (energy: string) => `insert into yearly (year, installed${energy}) values(?,?) on conflict(year) do update set installed${energy}=excluded.installed${energy};`,
        monthlyInstalled: (energy: string) => `insert into monthly (month, installed${energy}) values(?,?) on conflict(month) do update set installed${energy}=excluded.installed${energy};`,
        dailyBalance: (country: string) => `insert into daily (day, balance${country}) values(?,?) on conflict(day) do update set balance${country}=excluded.balance${country};`,
        monthlyBalance: (country: string) => `insert into monthly (month, balance${country}) values(?,?) on conflict(month) do update set balance${country}=excluded.balance${country};`,
        yearlyBalance: (country: string) => `insert into yearly (year, balance${country}) values(?,?) on conflict(year) do update set balance${country}=excluded.balance${country};`,
